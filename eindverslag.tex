...
Java biedt een standaard interface voor het modelleren van data voor tabellen. Wij hebben ervoor gekozen om van deze mogelijkheid gebruik te maken door ons SpreadSheet model te baseren op de Java klasse AbstractTableModel. Op deze manier wordt het dynamisch updaten van de GUI automatisch voor ons geregeld door Java. Hierdoor hoeven we enkel rekening te houden met de manier waarop wij onze data opslaan en teruggeven aan onze JTable. Wij hebben ervoor gekozen om alle SpreadSheet data op te slaan in een 1-dimensionale HashMap. Dit hebben wij geimplementeerd door een (x, y) coordinaat als volgt te vertalen naar een integer:\\

index = x * totalRows + y\\

Door het gebruik van een HashMap is het ophalen van Cellen, dankzij hun index, zeer snel. Verder is het bij een HashMap mogelijk om bijvoorbeeld indices 2 en 5 te vullen, en indices 1, 3 en 4 over te slaan. Hierdoor kan de HashMap minder Cellen bevatten dan er in de GUI getoond worden. Dit kan bij Arrays bijvoorbeeld niet. Dit komt de efficientie en het geheugengebruik goede.
...
Tijdens het implementeren van de basis van het project hebben we zoveel mogelijk rekening gehouden met efficientie en resource gebruik. Voor het ontwikkelen van de GUI is hier echter minder rekening mee gehouden. De oorzaak hiervan ligt grotendeels bij het feit dat het begrip van JTable en AbstractTableModel aan het begin van het project nog niet voldoende was om hier goed over na te kunnen denken. Tijdens het project hebben we hier echter goed genoeg kennis mee kunnen maken om hier wel rekening mee te kunnen houden. Een belangrijke verandering die dit teweeg heeft gebracht is het implementeren van Observers en Observables. Gedurende het project werd de GUI namelijk geupdate door bij elke setValue in ons SpreadSheet model de methode fireTableDataChanged aan te roepen, wat tot gevolg heeft dat de hele tabel opnieuw gerenderd dient te worden. Door het SpreadSheet data model een Observer te maken en door de Cell objecten een Observer en een Observable te maken hebben we dynamische updates van de GUI mogelijk gemaakt, zonder iedere keer de hele GUI te moeten renderen. Hier zaten helaas nog wat haken en ogen aan vast, aangezien Observers volgens hun isEquals() methode niet gelijk mogen zijn aan andere Observers. Dit heeft tot gevolg gehad dat we een flink deel van de Observer functionaliteit zelf hebben moeten implementeren.
...
Tegen het einde van het project liepen er een aantal dingen mis. Zoals al eerder in het verslag genoemd zijn we vrij laat begonnen met het implementeren van formules. Volgens de SCRUM planning zouden Dave en Joris zich hier voornamelijk mee bezig houden. De rest van het team heeft daardoor niet of nauwelijks naar formules gekeken. Aan het einde van het project bleek er echter nog veel werk verzet te moeten worden. Dit zorgde ervoor dat er op het laatste moment nog heel veel issues in de formule parser naar voren kwamen. Sommige van deze issues zijn te groot om op te lossen in de parser zoals deze nu is en vereisen eigenlijk een nieuw design. Tijdens de slotfase wist ik me daardoor af en toe geen raad meer met alle dingen die er tegelijk moesten gebeuren. Vanaf dat moment is mijn inzet minder geworden en had ik ook een stuk minder plezier in het project.
