...
public class LexerTest {
	...
	public void test_Constructor_FormuleBinaryOP() {
		...
		Token next = lex.next();
		assertTrue(next.type.equals(TokenType.BINARYOP));
	...
	public void test_ExprTwoArgs(String expr, String arg1, String arg2) {
		Token next;
		next = lex.next();
		assertTrue(next.type.equals(TokenType.WORD));
		assertTrue(next.data.equals(expr));
		next = lex.next();
		assertTrue(next.type.equals(TokenType.BRACKET));
		assertTrue(next.data.equals("("));
		next = lex.next();
		assertTrue(next.type.equals(TokenType.NUMBER));
		assertTrue(next.data.equals(arg1));
		next = lex.next();
		assertTrue(next.type.equals(TokenType.COMMA));
		assertTrue(next.data.equals(","));
		next = lex.next();
		assertTrue(next.type.equals(TokenType.NUMBER));
